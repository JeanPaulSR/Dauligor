    ...
    <div className={`${hideHeader ? '' : 'max-w-6xl mx-auto space-y-8 pb-20'}`}>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
              ...
              <form onSubmit={handleSave} className="flex flex-col h-full overflow-hidden mt-4">
                <div className="flex-grow overflow-y-auto custom-scrollbar space-y-4 pr-2">
                  <div className="space-y-1">
                    <label className="text-xs font-bold uppercase tracking-widest text-ink/40">Skill Name</label>
                    <Input 
                      value={name}
        ...
        <div className="lg:col-span-2 space-y-4">
          {loading ? (
            <div className="text-center py-10 font-serif italic opacity-50">Loading skills...</div>
          ) : skills.length === 0 ? (
            <div className="text-center py-20 border border-dashed border-gold/20 rounded-xl">
              <p className="text-ink/40 italic">No skills defined yet.</p>
            </div>
          ) : (
            <div className="grid gap-4">
              {skills.map(skill => (
                <Card key={skill.id} className="border-gold/10 bg-card/30 hover:border-gold/30 transition-all">
                  <CardContent className="p-4 flex items-start justify-between gap-4">
                    <div className="flex-grow space-y-1">
                      <div className="flex items-center gap-3">
                        <h3 className="font-serif font-bold text-xl text-ink uppercase tracking-tight">{skill.name}</h3>
                        {skill.identifier && (
