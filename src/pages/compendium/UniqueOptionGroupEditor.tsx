...
export default function UniqueOptionGroupEditor({ userProfile }: { userProfile: any }) {
  ...
  const [allScalingColumns, setAllScalingColumns] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribeScaling = onSnapshot(query(collection(db, 'scalingColumns'), orderBy('name')), (snap) => {
      setAllScalingColumns(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });

    // Fetch Features
    const unsubscribeFeatures = onSnapshot(query(collection(db, 'features'), orderBy('name')), (snap) => {
      setAllFeatures(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });
  ...
  const handleSaveItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem.id) {
        ...
        await addDoc(collection(db, 'uniqueOptionItems'), {
          ...itemData,
          createdAt: new Date().toISOString()
        });
      }
  ...
  const handleDeleteGroup = async () => {
    if (id && window.confirm('Delete this entire group and all its options?')) {
      try {
        // Note: In production, you'd want to delete items too or use a cloud function
      ...
      <div className="grid lg:grid-cols-3 gap-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input 
                      ...
                      onChange={e => setEditingItem(prev => ({ ...(prev || { levelPrerequisite: 0, isRepeatable: false }), levelPrerequisite: parseInt(e.target.value) }))}
                      className="h-8 text-sm bg-background/50 border-gold/10 focus:border-gold"
                    />
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <select 
                      value={editingItem?.featureId || ''} 
                      onChange={e => setEditingItem(prev => ({ ...(prev || { levelPrerequisite: 0, isRepeatable: false }), featureId: e.target.value }))}
                      className="w-full h-8 px-2 rounded-md border border-gold/10 bg-background/50 focus:border-gold outline-none text-sm"
                    >
                      <option value="">None</option>
                      {allFeatures.map(f => (
                        <option key={f.id} value={f.id}>{f.name} (L{f.level} {classes.find(c => c.id === f.parentId)?.name || 'Unknown Class'})</option>
                      ))}
                    </select>
                  </div>
        ...
        <div className="space-y-6">
          <div className="p-4 border border-blood/20 bg-blood/5 space-y-4">
            <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-blood border-b border-blood/10 pb-2">Danger Zone</h2>
            <Button 
              variant="ghost" 
              size="sm"
