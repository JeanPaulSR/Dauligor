      ...
      <div className="section-header">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {isCharacterDM && <p className="text-sm text-ink/65 font-bold tracking-widest uppercase">Archive Administration View</p>}
          </div>
        </div>
        <Dialog>
          ...
          <DialogContent className="sm:max-w-5xl bg-background border-gold/35 p-0 shadow-2xl overflow-hidden rounded-xl">
            <div className="bg-gold/5 border-b border-gold/15 p-4 sm:p-6 transition-all">
              <DialogHeader>
            ...
            <div className="p-4 sm:p-6 md:p-8 space-y-4">
              <p className="text-center text-ink/65 font-serif translate-y-[-5px] sm:translate-y-[-10px] italic text-xs sm:text-sm px-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div 
                  ...
                  className="p-4 sm:p-6 border border-gold/25 bg-card/40 rounded-lg relative group flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-8 transition-all hover:bg-gold/15 cursor-pointer hover:border-gold/55 shadow-sm min-h-[100px] sm:min-h-[140px] text-center sm:text-left"
                >
                  <div className="w-16 h-16 sm:w-20 sm:h-20 flex-shrink-0 bg-gold/15 rounded-lg flex items-center justify-center border border-gold/25 text-gold shadow-sm group-hover:scale-105 transition-transform duration-500">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <p className="text-xs sm:text-sm text-ink/75 font-medium max-w-2xl leading-relaxed">
                ...
                <div 
                  ...
                  className="p-4 sm:p-6 border border-gold/25 bg-card/40 rounded-lg relative group flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-8 transition-all hover:bg-gold/15 cursor-pointer hover:border-gold/55 shadow-sm min-h-[100px] sm:min-h-[140px] text-center sm:text-left opacity-90"
                >
                  <div className="w-16 h-16 sm:w-20 sm:h-20 flex-shrink-0 bg-gold/15 rounded-lg flex items-center justify-center border border-gold/25 text-gold shadow-sm group-hover:scale-105 transition-transform duration-500">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex items-center justify-center sm:justify-start gap-2 pt-1">
                       <div className="hidden sm:block h-[1px] w-4 bg-gold/35" />
                       <p className="text-[10px] sm:text-[11px] text-gold/65 font-black uppercase tracking-widest italic">
              ...
              <div className="flex justify-center sm:justify-end pt-6">
                <DialogClose render={<Button variant="outline" className="border-gold/25 text-ink/65 hover:text-ink hover:bg-gold/5 font-bold uppercase tracking-widest text-[10px] px-6 h-9">
                  Cancel
                </Button>} />
              </div>
            </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {characters.map(char => (
          <Link key={char.id} to={`/characters/builder/${char.id}`}>
            <div className="border border-gold/25 bg-card/40 p-6 rounded hover:bg-gold/5 transition-all group flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <h2 className="font-serif font-bold text-xl text-ink group-hover:text-gold transition-colors">{char.name}</h2>
                <div className="w-8 h-8 rounded border border-gold/35 bg-gold/15 flex items-center justify-center font-bold text-gold">
                  {char.level || 1}
                </div>
              </div>
              <div className="text-xs text-ink/55 uppercase tracking-widest font-bold">
                {char.campaign_id ? 'Assigned to Campaign' : 'No Campaign'}
              </div>
              {isCharacterDM && char.user_id !== userProfile.id && (
                <div className="mt-2 pt-2 border-t border-gold/15 flex items-center gap-2 text-xs text-ink/45">
        ...
        {characters.length === 0 && (
          <div className="col-span-full py-20 text-center border border-dashed border-gold/25 rounded">
            <p className="text-ink/45 italic">You haven't created any characters yet.</p>
          </div>
        )}
      </div>
    </div>
