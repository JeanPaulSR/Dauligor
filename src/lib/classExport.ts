...
// `./d1` at all.
import { normalizeSpellFormulaShortcuts, normalizeSemanticReferenceText, type ReferenceColumnExample } from './referenceSyntax';
import {
...
// (string) depending on the fetcher; this normalizes both.
...
// spell-sourceId allowlist for the module-side walker. Defined here
// (not in `requirements.ts`) because it's purely an export-pipeline
// concern.
...
function normalizeSpellcastingForExport(spellcasting: any, refs: any = {}, {
  ...
  // `spellcastingTypes` is fetched via `fetchCollection` without going
  // through `denormalizeCompendiumData`, so rows arrive with their raw
  // D1 column names. Read `foundry_name` verbatim — no `?? foundryName`
  // fallback (see docs/database-memory.md §2 — no dual-shape reads).
  // Reading the camelCased form silently returned undefined for every
  // row, which DELETED `progression` from every export and made every
  // imported class look like `progression: "none"` on the actor side.
  const mappedProgression = trimString(progressionType?.foundry_name).toLowerCase();
...
export async function exportClassSemantic(
  ...
  if (referencedSpellRuleIds.size > 0) {
    ...
    // Pull only the columns the matcher needs. Single SELECT over the spells
    // table — cheaper than per-rule re-fetches, even for catalogs in the
    // low thousands.
  ...
  // The per-class spell list is no longer baked into this bundle.
  // It lives on its own endpoint
  // (`/api/module/<source>/classes/<class>/spells.json`) served live
  // from D1 — the Foundry module fetches it separately during class
  // import. Decoupling means spell-list edits flow through without a
  ...
  return {
    ...
    spellRuleNameById,
    // NOTE: the per-class spell list (formerly `classSpellItems`) is
    // no longer part of the class bundle. The Foundry module fetches
    // it from a separate live endpoint — see
