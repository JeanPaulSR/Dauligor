...
export function ActivitySection({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <section className="mt-5 first:mt-0">
      <header className="flex items-center gap-2 mb-1 px-3 py-2 rounded-t bg-gold/5 border border-gold/15 border-b-0">
        <span className="w-1 h-3 bg-gold/65 rounded-sm shrink-0" aria-hidden />
        <h3 className="text-[10px] uppercase tracking-[0.2em] font-black text-gold/85 select-none">{label}</h3>
      </header>
  ...
  return (
    <div className={cn('flex gap-4 py-2.5', inline ? 'items-center' : 'items-start')}>
      <div className="flex-1 min-w-0">
        <p className="text-xs font-semibold text-ink/85 leading-none">{label}</p>
