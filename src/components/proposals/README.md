...
| [`useProposalPreFlushSave({...})`](../../hooks/useProposalPreFlushSave.ts) | Registers `handleSave` as the wrapper's pre-flush callback (so Submit Changes stages the current form before draining the queue). Variants for single-work (gate on `effectiveId`) and catalog (closure with dirty-check). |
