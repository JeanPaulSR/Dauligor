...
export default function ReferenceSyntaxHelp({
  ...
  return (
    <details className="group border border-gold/15 bg-background/30 rounded-md overflow-hidden mt-2">
      <summary className="flex items-center gap-2 px-3 py-2 cursor-pointer select-none bg-background/50 hover:bg-background/80 transition-colors">
        <BookOpen className="w-3.5 h-3.5 text-gold/75" />
        <span className="text-[10px] font-bold uppercase tracking-widest text-gold/75 group-hover:text-gold transition-colors">{buttonLabel}</span>
      </summary>
      <div className="p-3 space-y-3 border-t border-gold/15 bg-background/20">
        ...
        {hasPreview && (
          <div className="grid gap-2 md:grid-cols-2">
            <div className="rounded-md border border-gold/15 bg-background/35 p-3">
            ...
            <div className="rounded-md border border-gold/15 bg-background/35 p-3">
              <div className="flex items-center gap-2">
                ...
                {previewChanged && <Sparkles className="w-3 h-3 text-gold/65" />}
              </div>
            ...
            {rows.slice(0, examples && examples.length > 0 ? undefined : 6).map((row) => (
              ...
              >
                <div className="min-w-0">
                  <p className="text-[10px] font-black text-ink/85">{row.label}</p>
                  <p className="mt-1 text-[10px] leading-relaxed text-ink/45">
                    {row.description}
                  </p>
                </div>
                <code className="text-[10px] text-gold break-all">{row.semantic}</code>
                <code className="text-[10px] text-gold/85 break-all">{row.native}</code>
              </div>
            ))}
          </div>
        </div>
