...
Branch is now **25 ahead / 53 behind origin/main** (fetched 2026-06-07 — `main` has advanced well beyond the 2026-06-06 snapshot's "12 behind", so a rebase onto `origin/main` will be substantial; do it before any merge/push). Still local-only, NOT pushed. The class mark-up panel was reworked into the user's **section model**. New commits (newest first):

- `d63a09b` **Bulk feature paste + subclass-placeholder row + spellcasting confirm.** (1) Features section takes the WHOLE blob → `assignAppendMany`/`classParse.splitFeatures` → one draft per heading, kinds auto-routed (over-split → merge in panel; exact → Mark-text Feature mark). (2) A feature routed to **subclass** now writes a real `features` row with `is_subclass_feature=1` (name=subclass_title, first subclass level) — verified in local D1 (Grave Ambition L3 =1); `routeFeatures` emits it, `commit` passes `isSubclassFeature` (compendium.ts maps → column). (3) **Spellcasting** = route a feature to Spellcasting → `buildSpellcastingConfig` sets ability/type/ritual/description from text; `progressionId`/`spellsKnownId` are FKs into `spellcastingScalings`/`scaling_columns` — NOT text-derivable, left empty for the editor dropdowns. (Branch now ~26 ahead.)
- `1372e1e` **"Paste by section" drop-zones (DEFAULT class input).** Classes open into per-block boxes via "Enter by section →" (blank start): one drop-zone per Block-group target (Name/Description/Flavour/Proficiencies/Equipment/Multiclassing) + a Features repeater. Each box runs the SAME interpreter on blur (assignField/assignResolve; Proficiencies box resolves the whole grid + hit die/saves/primary). Left panel toggles **Sections ⇄ Mark text** (`leftMode`, default 'sections' when a 'Blocks' group exists). New components in the window: `SectionsPanel`/`BlockDropZone`/`FeatureDropZone`. Spells unchanged (no Block group). Note: boxes apply on **blur** (React delegates onBlur via `focusout` — dispatch `focusout`, not `blur`, when scripting).
- `1c819ee` **Popover on-screen fix.** The assign popover was anchored to a blurred page wrapper (`filter: blur(0)` from the entry animation → becomes the containing block for `position:fixed`), rendering off-screen for long text. Fixed by **portaling the popover to `document.body`** (`react-dom` createPortal) + viewport-aware placement (flip above when low, cap maxHeight to available space + overflow-auto). `ActiveSelection.selTop` added.
