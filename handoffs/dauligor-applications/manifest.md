# Branch: `dauligor-applications`

Started: 2026-05-31 (continues the work of the retired `system-applications` branch; folder renamed to match)
Owner: Claude
Goal: Multi-step. **Next direction**: (1) admin pages pass; (2) bring the home-block system to Articles using the campaign-home builder as the baseline. Original branch scope (carried over): Phase 1.5 hash-on-upsert, article-system schema revamp, Phase 2 live viewer of the live-content bridge.
Status: **active** (created 2026-05-31 from `origin/main`; pushed to origin — has a remote backup, which `system-applications` never did)

> **Read first:** [2026-05-31-fresh-start-brief.md](2026-05-31-fresh-start-brief.md) — project orientation + owner working-style + the branch-reset record. Newest handoff wins.
...
- 2026-05-29 — [2026-05-29-system-pages.md](2026-05-29-system-pages.md) — System page article type shipped — `system_pages` + `system_page_entries` migration (local + **remote applied 2026-05-29**), data layer, reader (`/system/:identifier`, Quiet-Focus blocks + right Contents rail + StrictMode-safe deep-link), master-detail admin (sticky save, `/new` opens editor directly), `&kind[]` page-level refs + Foundry `&Reference[…]` pre-pass + name-slug aliases. Commits `8989bd1` (feature) + `1ecbf0a` (refs wiring), both on `origin/main`.
- 2026-05-30 — [2026-05-30-campaign-home-builder.md](2026-05-30-campaign-home-builder.md) — Per-campaign homepage layout builder shipped to `origin/main` (original builder `a2ca53f`; HEAD now `b59cba0` after a 2026-05-30 editor-feedback iteration — Hero→Header + BBCode subtitle, per-card `title`/`description`/`span` overrides (replacing `featureFirst`), columns-as-labeled-sections via a new `column` block type, BBCode callout/description boxes, `callout`+`column` added to the server `ALLOWED_HOME_BLOCK_TYPES`, and a helper-class styling pass; see the handoff's "Iteration update"). Fullscreen 3-pane Outline+Preview editor at `/campaign/edit/:id/homepage` (nested containers, generalized entity rows reusing references.ts, placeholder refs, resizable panes). `Home.tsx` unified onto the block render path so the default survives save (preview == saved). New: `src/lib/campaignHome.ts`, `src/components/campaign/{CampaignHomeBlocks,CampaignHomeEditor}.tsx`, `src/pages/campaign/CampaignHomeEditorPage.tsx`, migration `20260529-1700_campaign_home_blocks.sql` (**LOCAL + REMOTE APPLIED 2026-05-30** — the missing remote table was causing a prod `500` on `GET …/home-blocks`; now fixed), API `GET`/`PUT /api/campaigns/:id/home-blocks`. Also fixed this session: a homepage crash from a missing `callout` parse case in `parseHomeBlock` (commit `fbd3c92`). Also earlier: system-pages migration → remote, `docs/architecture/cross-references.md`, Maps fixes + `docs/features/maps.md`. Next: user retry image upload (stale-server fix), logged-in e2e, backgrounds as a ref kind.
- 2026-05-31 — [2026-05-31-fresh-start-brief.md](2026-05-31-fresh-start-brief.md) — **CURRENT**: branch reset. A password-reset attempt (self-service change-password + admin reset dialog + post-login nudge) was reverted off `origin/main` and the 7 attempt commits parked on `wip/password-reset-attempt-2026-05-31`. `system-applications` was retired and replaced by `dauligor-applications` (created from `origin/main`, pushed to origin). This brief doubles as a full project + working-style orientation. No feature code changed; `origin/main` and the branch are both at `fff679b`.
