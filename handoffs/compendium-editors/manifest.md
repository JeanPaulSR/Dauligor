...
- [x] `(2026-05-28)` Request `proposal-system` to decide cross-reference **scope** before I build Parts B + C: (1) features-as-proposable; (2) the guard-#1 reference-walk gaps; (3) `scaling_column` `parent_type`. Full request: [2026-05-28-open-request-to-proposal-system.md](2026-05-28-open-request-to-proposal-system.md). **RESOLVED** — they went full-scope ([2026-05-28-proposal-system-reply.md](2026-05-28-proposal-system-reply.md)): features IN, full coverage, all six `parent_type`s. B/C unblocked + underway.
- [ ] `(2026-05-29)` Follow-up to `proposal-system` after Part B shipped + a user spot-check: [2026-05-29-followup-to-proposal-system.md](2026-05-29-followup-to-proposal-system.md). **R1** — `scaling_column.writableColumns` missing `type`/`identifier`/`distance_units` (their one-line fix; proposed columns lose them on approval). **R2** — confirm guard #1 + atomic approve resolve in-block `scaling_column`/`feature` creates whose `parent_id` is a same-block draft (esp. the feature graph: `feature.parent_id` → draft class/subclass, feature advancement refs, `unique_option_groups.feature_id`). **R3 (foundational, user directive)** — gate proposal-editor entry behind an active block (`PickOrCreateBlockDialog`) so content-creators can't author composite content in standalone mode. Plus FYI: "submitted for review" toast doesn't distinguish block-queue from standalone; list/panel draft overlays (we're fixing our side).
