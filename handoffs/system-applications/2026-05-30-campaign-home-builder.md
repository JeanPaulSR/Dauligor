...
- **DB**: `worker/migrations/20260529-1700_campaign_home_blocks.sql` — one row per ROOT block (`id, campaign_id, block_type, "order", config`); nesting lives inside `config.children`. **LOCAL applied; REMOTE APPLIED 2026-05-30** (with per-migration go-ahead — `cd worker && npx wrangler d1 execute dauligor-db --remote --yes --file=…`; verified the `campaign_home_blocks` table + `idx_campaign_home_blocks_campaign_order` index exist on prod D1). ⚠ Its `--` header comments are STALE (describe the original `article-row {title,columns,articleIds[]}` design); the `CREATE TABLE` itself is correct and type-agnostic (`block_type TEXT` + `config TEXT` JSON), so it stores today's block types fine — only the comments need a follow-up fix.
...
2. **Remote migration** `20260529-1700_campaign_home_blocks.sql` — ✅ **APPLIED to remote 2026-05-30.** This was the cause of a production `500` on `GET /api/campaigns/:id/home-blocks`: the handler `SELECT`s from `campaign_home_blocks`, which didn't exist on prod D1 (verified absent, then created). Endpoint now returns 401 (auth gate) instead of 500. Lesson: a feature's remote migration must be applied before/with its code shipping, or its endpoints 500 in prod even while the client code is fine.
