...
- 2026-05-30 — [2026-05-30-campaign-home-builder.md](2026-05-30-campaign-home-builder.md) — **CURRENT**: Per-campaign homepage layout builder shipped to `origin/main` (HEAD `a2ca53f`). Fullscreen 3-pane Outline+Preview editor at `/campaign/edit/:id/homepage` (nested containers, generalized entity rows reusing references.ts, placeholder refs, resizable panes). `Home.tsx` unified onto the block render path so the default survives save (preview == saved). New: `src/lib/campaignHome.ts`, `src/components/campaign/{CampaignHomeBlocks,CampaignHomeEditor}.tsx`, `src/pages/campaign/CampaignHomeEditorPage.tsx`, migration `20260529-1700_campaign_home_blocks.sql` (**LOCAL only — remote PENDING go-ahead, verified absent on remote**), API `GET`/`PUT /api/campaigns/:id/home-blocks`. NOT yet verified logged-in (auth-gated). Also this session: system-pages migration → remote, `docs/architecture/cross-references.md`, Maps fixes + `docs/features/maps.md`. Next: user retry image upload (stale-server fix), apply remote migration, logged-in e2e, backgrounds as a ref kind.
