...
// CORS so the Foundry module (a cross-origin browser client) can read campaign
// content with a Bearer token. Mirrors the wrapper /api/module/* uses;
// Bearer-token auth, no cookies, so `*` is safe.
const CORS_HEADERS = {
...
export const onRequest = async (context: any): Promise<Response> => {
  ...
  const response = await handleCampaignsRequest(context);
...
const handleCampaignsRequest = async (context: any): Promise<Response> => {
