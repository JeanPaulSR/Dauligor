-- Phase 4 — add `subclass` to `pending_revisions.entity_type` allowlist
-- ============================================================
-- Same SQLite "can't ALTER a CHECK in place" rebuild as 20260519-1600.
-- Extends the entity allowlist with `subclass` so the SubclassEditor
-- can submit drafts through the proposal pipeline.
--
-- ENTITY_CONFIGS in `api/_lib/proposals.ts` already has the
-- matching `subclass` entry (writable columns + JSON-column markers)
-- so payload sanitisation will pass once this migration runs.
