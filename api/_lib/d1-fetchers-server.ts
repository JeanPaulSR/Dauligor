...
// Used today by `server.ts` (local Node, no Vercel bundler) so the local-
// mode `/api/module/<slug>/classes/<identifier>.json` endpoint returns the
// full semantic bundle. The Vercel-deployed `api/module.ts` cannot import
// `src/lib/classExport.ts` cleanly yet (cross-folder bundling crash) so it
// still returns a flat row in production.
//
// The collection-name → table-name map is inlined here rather than imported
// from `src/lib/d1Tables.ts`. Keeps this module dependency-free of `src/`,
// which avoids the same cross-folder issue if api/module.ts ever picks it
// up. Keep this in sync with `src/lib/d1Tables.ts` when tables are added.
