...
| Auth | **Native session tokens** — Worker-issued HS256 JWT (scrypt passwords in D1, login via `/api/auth/login`). Firebase Auth remains only as a **migration-window fallback**, removed in Phase 5. |
...
1. **Never** add `firebase/firestore` imports anywhere. The database is gone — `getDoc`, `getDocs`, `addDoc`, `setDoc`, `updateDoc`, `deleteDoc`, `onSnapshot`, `collection(db, ...)` are all forbidden. Use the helpers in [src/lib/d1.ts](src/lib/d1.ts): `queryD1`, `batchQueryD1`, `fetchCollection`, `fetchDocument`, `upsertDocument`, `upsertDocumentBatch`, `deleteDocument`, `deleteDocuments`. Auth no longer runs on Firebase — the app issues its **own native session tokens** (Worker-signed HS256 JWT, login via `/api/auth/login`), so the remaining `firebase/auth` import is a **migration-window fallback only** (removed in Phase 5), not the auth layer — see [docs/platform/auth-firebase.md](docs/platform/auth-firebase.md).
