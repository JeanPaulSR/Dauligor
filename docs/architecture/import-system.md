...
- **`EntityWorkspace`** — the workspace body. At **xl+** it's a fullscreen **three-column** layout: the annotated source / drop-zone **Sections** (left — gold-highlighted per span, hover-link, **select-to-assign** popover) · the structured **Fields** (middle) · an always-on **Live preview** (right). Each column scrolls internally (the page itself doesn't); below xl the columns stack and the page scrolls. Reused by single mode and each batch candidate's *Review*.
...
- **Format templates** — `Save as format` (spell-only) captures `target → line-index` from one marked-up entity into `localStorage['dauligor:importFormat:<type>:<source|default>']`; auto-applied over the heuristic parse on later interprets.
- **`renderPreview`** — the always-on Live-preview column. Type-aware: **spell** feeds the richer `SpellDetailPanel`; **every other type** renders the unified `EntityPreview` sheet (name · key stats · the child features/options list, from the resolved payload). A new type previews for free — only a richer, bespoke detail pane needs its own branch.
...
| option group | `upsertDocument('uniqueOptionGroups', id, d1Data)` (name / description / source_id / class_ids — **no `identifier`, no `queueRebake`**) + a child `upsertDocument('uniqueOptionItems', …)` per option (`group_id`, `feature_type` = the group name, empty-default deep shapes) |
| species / background / facility / source / system pages / spell_rules | direct `upsertDocument(collection, id, payload)` (or `saveRule` / `saveSystemPage*`) |
...
`ImportFieldDef` kinds the window renders: `text`, `textarea`, `number`, `boolean`, `select` (needs `options`), and `source` (a persistent `<select>` from the `sources` table, rendered top-level and shared across creates). The class/subclass importers add richer kinds, each reusing the matching editor control: `markdown` (the `MarkdownEditor` TipTap editor — value carries site BBCode, handles rich paste), `abilities` (ability-pill toggles, value = UPPERCASE ability ids), `proficiencies` (the reusable `ProficienciesEditor` grid; value is the class proficiency object), `features` (the feature organizer — merge / collapse / re-route drafts), `parentClass` (a `SingleSelectSearch` of existing classes for a subclass's parent; value is `{ id, identifier, name }` so `buildPayload` can fill both `class_id` and `class_identifier`), and `classMulti` (an `EntityPicker` multi-select of classes; value is a `string[]` of ids — an option group's class restrictions). A `features` field can pass `featureKinds` to relabel the draft-kind dropdown — an option group shows **Option / Skip** instead of feature/spellcasting/asi/subclass/skip. Group fields with `group` ("Identity", "Mechanics", …) → they render as `.config-fieldset` sections. `key` must match the camelCase form-state key `buildPayload` reads, so the payload stays a faithful mirror of the editor.
...
4. **Preview** — automatic: the unified `EntityPreview` sheet renders any non-spell type from its resolved payload (name · stats · features/options). Only add a `renderPreview` branch (like spell's `SpellDetailPanel`) if the type needs a richer bespoke detail pane.
...
- **Preview + overwrite** — the always-on Live preview renders the subclass via the unified `EntityPreview` sheet (parent class + features). The **Overwrite** picker is type-aware (it loads the current descriptor's `collection`, so subclass overwrites subclasses).

**Folded sub-headers render as flat headings, not bold.** When the feature pipeline folds a sub-section header (e.g. "Cantrips") into a feature body — during parse, or via the Features panel's merge / collapse — it emits `[h3]Name[/h3]` on its own line with a blank line after (a flat site heading via `bbcodeToHtml`, sitting in its own block so it can be re-tagged/edited), **not** `[b]…[/b]`. Route every folded header through the shared `subHeadingBBCode` helper + `SUBHEADING_SPLIT_RE` (exported from `classParse.ts`) so the merge ↔ split round-trip stays exact. `SUBHEADING_TAG` is the single knob for the level. `[h3]` is only ever a **subsection inside a body** — feature / option NAMES are stripped of all BBCode tags (`stripBbcodeTags`, exported from `classParse.ts`) at parse / display / commit, so a `[h3]`-wrapped header (e.g. from an HTML paste) never leaks into the name.

### Third worked type — option group (`src/lib/import/uniqueOptionGroup.ts`)

The class→features shape again, for **modular option groups** (Eldritch Invocations, Fighting Styles, Metamagic, Maneuvers, Infusions) — what classes draw their choices from, so it "rounds out" class imports.

- **Group (parent) + option items (children).** `commit` writes the group then one `uniqueOptionItems` row per option (see the fidelity table). `feature_type` is locked to the group name; the deep structure (requirements tree, activities, uses) stays in the editor (skeleton scope). **No `queueRebake`** — the option-group editor doesn't rebake.
- **Reuses the feature pipeline** via the `features` field kind, relabeled **Option / Skip** with `featureKinds`. Options are split **without folding** (`splitOptions` calls `splitClassSections` directly — the class fold rule would collapse level-less options into the first); the level **prerequisite** is sniffed from a "Prerequisite: 9th level" line.
- **Class restrictions** are an optional `classMulti` field → `class_ids`. ⚠️ `source_id` is a FK to `sources(id)`, so an unsourced group writes **`null`**, never `''` (which FK-fails) — the one gotcha that differs from class/subclass.
