...
│     - api/_lib/firebase-admin.ts: verify JWT (jose+JWKS) +      │
│                                   Identity Toolkit REST for     │
│                                   admin ops; check role         │
...
|---|---|---|
| App server | Express via `tsx watch server.ts` (port 3000) | Cloudflare Pages Functions |
| Vite dev | Express mounts Vite middleware | Static `dist/` served by Pages CDN |
...
| Env vars | `.env`, `worker/.dev.vars` | Pages env (Production + Preview), Worker secrets |
| `R2_WORKER_URL` | `http://localhost:8787` | `https://dauligor-storage.<account>.workers.dev` |

The Express dev server in `server.ts` and the Pages Functions in `functions/api/**` share their underlying logic via `api/_lib/`. So the proxy/auth surface area is identical between dev and prod for most routes — local dev does **not** drift, **except** `/api/module/*`, which only exists as a Pages Function. For local module-export testing run `npx wrangler pages dev` instead of (or alongside) `npm run dev`.
...
2. The Pages Function at [functions/api/characters/[id].ts](../../functions/api/characters/[id].ts) runs `requireCharacterAccess(authHeader, characterId)`, which:
   - Verifies the JWT.
...
All seven R2 actions (list / delete / rename / move-folder / upload / scan-references / rewrite-references) live in one dispatcher at [functions/api/r2/[action].ts](../../functions/api/r2/[action].ts). The consolidation was originally driven by the Vercel Hobby 12-function cap; on Pages the cap doesn't exist but keeping the actions co-located still helps cohesion.
...
| [functions/api/](../../functions/api/) | Cloudflare Pages Functions in prod | Native handlers for every `/api/*` route the SPA hits |
