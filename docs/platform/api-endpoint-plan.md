...
| #8 — campaign writes through generic proxy | ✅ Closed | api/campaigns.ts gained POST/PATCH/DELETE + PUT/DELETE members routes. `isCharacterDM` (admin + co-dm) gates create/update/member-add/member-remove; delete additionally re-requires admin. The proxy refuses direct `campaigns` / `campaign_members` writes with a 403 pointing at the per-route endpoint. Closes the lore-writer-can-edit-any-campaign vector the audit's #8 entry flagged. |
...
| M2 — `users` table fully enumerable (PII) for staff | ✅ Closed | `api/admin/users.ts` ships GET (column-scoped: admin gets `recovery_email`, other staff get the basic set), POST (create via Firebase Admin SDK), PATCH (allow-listed updates + `campaign_ids` reconciliation), DELETE (Firebase Auth + D1 cascade), plus the `[id]/temporary-password` and `[id]/sign-in-token` actions. Proxy refuses direct SELECT against `users` via `PROTECTED_READ_TABLES`. |
